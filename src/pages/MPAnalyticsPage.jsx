...
const MPAnalyticsPage = () => {
    ...
    // Get MPs list based on house filter
    const mpsList = useMemo(() => {
        return getMPsByHouseForAnalytics(selectedHouse)
    }, [selectedHouse])

    // Auto-select first MP when list changes, unless an MP was passed in state
    useEffect(() => {
        if (mpsList.length > 0 && !selectedMP) {
            setSelectedMP(mpsList[0].name)
        }
    }, [mpsList, selectedMP])

    // Get spending breakdown for selected MP
    const mpData = useMemo(() => {
        if (!selectedMP) return null
        return getMPSpendingBreakdown(selectedMP)
    }, [selectedMP])
